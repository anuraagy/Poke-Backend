class Api::V1::UsersController < Api::V1::BaseController
  ...
  def friend_requests_sent
    ...
    if user.blank?
      ...
      render status: :ok, json: { friend_requests: user.friend_requests_sent.where.not(status: "accepted").includes(:receiver).slice(:name, :id).as_json }
    end
  end
  ...
  def friend_requests_received
    ...
    if user.blank?
      ...
      render status: :ok, json: { friend_requests: user.friend_requests_sent.where.not(status: "accepted").includes(:sender).slice(:name, :id).as_json }
    end
  end
